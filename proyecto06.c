...
struct deptos * inicializarArtV(struct deptos *d,int N);
...
int main()
{
	...
	d=datosArticulos(d,d[0].tamanioD);
	d=inicializarArtV(d,d[0].tamanioD);
	do{
...
struct deptos * nombreDeptos(struct deptos *d)
{
	...
	while(respuesta!=1){
		...
		d[0].tamanioD++;
		printf("\nTermino de ingresar deptos:\n1)Si\n2)No:  ");
		scanf("%d",&respuesta);
		getchar();
		printf("\n tamanioD = %d",d[0].tamanioD);
		d=(struct deptos *)realloc(d,sizeof(struct deptos)*(d[0].tamanioD+1));
		i++;
	}
	free(aux);
	return d;
...
struct deptos * articulosPorDeptoC(struct deptos *d,int nDep,int nArt,int piezasc)
{
	...
	d[nDep-1].ArtCom=(char**)realloc(d[nDep-1].ArtCom,sizeof(char*)*(d[nDep-1].conc));
...
struct deptos * importarArticulosA(struct deptos *d)
{
	...
	while(feof(f)==0)
	{
		if(feof(f)==0){
			...
			printf("\nJalando: ");
			lineas=(char**)realloc(lineas,sizeof(char*)*(i+1));
